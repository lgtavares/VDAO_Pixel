...
random_seed = 142
old_database = '/home/luiz.tavares/Workspace/VDAO_Pixel_old/dataset/VDAO/'


class VideoLoader:

    def __init__(self, video_idx=1, database_dir=old_database, alignment=None):
        ...
        self.ref_video_path = database_dir + 'ref/{0:02d}.avi'.format(
            self.ref_idx)
        self.tar_video_path = database_dir + 'obj/{0:02d}.avi'.format(
            self.video_idx)
        self.sil_video_path = database_dir + 'sil/{0:02d}.avi'.format(
            self.video_idx)
    ...
    def _get_frame(self, frame_number, video_type):
        ...
        if video_type == 'reference':
            # Como arquivo de anotação csv tem o frame 0 como o primeiro,
            # obtém o frame normalmente
    ...
    def get_frame(self, ref_frame, tar_frame):
        ...
        ref_thread = self.executor.submit(self._get_frame, ref_frame,
                                          'reference')
        tar_thread = self.executor.submit(self._get_frame, tar_frame, 'target')
        sil_thread = self.executor.submit(self._get_frame, tar_frame,
                                          'silhouette')

        return (ref_thread.result(), tar_thread.result(), sil_thread.result(),
                self.alignment)
