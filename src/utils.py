...
def get_fold(filename, fold_number):
    assert fold_number >= 1 and fold_number <= 72, 'Fold number should be greater than 1 and less than 72.'
...
def apply_morphology(img_in, open_size=3, close_size=3):
    ...
    img_morph = [cv2.threshold(img, 50, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)[1] for img in img_morph]
    img_morph = [cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel_open) for img in img_morph]
    img_morph = [cv2.morphologyEx(img, cv2.MORPH_CLOSE, kernel_close) for img in img_morph]
...
def opening(vid, value):
    ...
    kernel_opening = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(value,value))
    return np.array([cv2.morphologyEx(vid[f,:,:], cv2.MORPH_OPEN, kernel_opening) for f in range(vid.shape[0])])

def closing(vid, value):
    
    kernel_closing = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(value,value))
    return np.array([cv2.morphologyEx(vid[f,:,:], cv2.MORPH_CLOSE, kernel_closing) for f in range(vid.shape[0])])

def erosion(vid, value):
    
    kernel_erosion = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(value,value))
    return np.array([cv2.morphologyEx(vid[f,:,:], cv2.MORPH_ERODE, kernel_erosion) for f in range(vid.shape[0])])

def dilation(vid, value):
    
    kernel_dilation = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(value,value))
    return np.array([cv2.morphologyEx(vid[f,:,:], cv2.MORPH_DILATE, kernel_dilation) for f in range(vid.shape[0])])
...
def apply_optical_flow(ref_image,
    ...
    if transformations is not None:
        ref_image = transformations(Image.fromarray(ref_image)).squeeze().permute(1, 2, 0)
        ref_image = (255 * ref_image.numpy()).astype(np.uint8)
        tar_image = transformations(Image.fromarray(tar_image)).squeeze().permute(1, 2, 0)
    ...
    # deslocamento vertical (v) e horizontal (u) considera que a ref_frames é a deslocada
    v, u = optical_flow_tvl1(tar_image_gs, ref_image_gs, tightness=0.1, num_warp=20, tol=1e-7)
    ...
    if apply_ransac:
        ...
        dst = np.array(((row_coords + u).flatten(), (col_coords + v).flatten())).T
        ...
        # Calcula RMS entre o frame alvo grayscale e o de referência transformado e em grayscale e desconsiderando
        rms_ransac = calculate_SRMSE(tar_image_gs,
                                     ref_ret_ransac_gs,
        ...
        #_ref_image_no_border = disconsider_borders(ref_image_gs, border_percentage_removal)
        #rms_ransac = calculate_SRMS(abs_diff_noborders) / _ref_image_no_border.std()
        # Se foi passado algum bounding box, calcula o RMS desconsiderando a area do bounding box
        ...
        img_vectors_ransac, model_ransac, inliers, rms_ransac, rms_ransac_outside_bb_only, abs_diff_noborders, ref_ret_ransac_gs, repositioned_bb = None, None, None, None, None, None, None, None
...
def conf_mat(x, y):
    totaltrue = np.sum(x)
    return conf_mat_opt(x.astype(int), y.astype(int), totaltrue, len(x) - totaltrue)
