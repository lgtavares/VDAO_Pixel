...
normalize_transform = transforms.Normalize(mean=resnet.MEAN_IMAGENET,
                                           std=resnet.STD_IMAGENET)
...
class VDAODataset(Dataset):
    ...
    def __init__(self,
        ...
        for fold_num in fold_set:
            # Get alignment files
            algn_file = os.path.join(
                PROJECT_DIR, 'data/alignment',
            ...
            # Get list of files
            self.tar_list.extend([
                os.path.join(self.tar_path, 'fold{0:02d}'.format(fold_num),
                             '{0:04d}.png'.format(idx))
                for idx in list(self.align_df.index)
            ])
            self.ref_list = [
                ii.replace(self.tar_path, self.ref_path)
                for ii in self.tar_list
            ]
            self.sil_list = [
                ii.replace(self.tar_path, self.sil_path)
                for ii in self.tar_list
            ]
    ...
    def __getitem__(self, idx):
        ...
        if (self.alignment == 'geometric') | (self.alignment == 'warp'):
            tar_frame = self._crop(tar_frame, ref_frame.shape[0],
                                   ref_frame.shape[1])
            sil_frame = self._crop(sil_frame, ref_frame.shape[0],
                                   ref_frame.shape[1])
