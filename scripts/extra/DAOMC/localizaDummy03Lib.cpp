...
void detect_feature_extract_descriptor(int siftOrSurf, int minHessian, Mat &img_reference, std::vector<KeyPoint> &keypoints_reference, Mat &descriptors_reference, Mat &img_target, std::vector<KeyPoint> &keypoints_target, Mat &descriptors_target)
{
	if (siftOrSurf == 0)
	{
	...
	else if (siftOrSurf == 2)
	{
		Ptr<BRISK> briskDetector = BRISK::create(10, 3, 1.0f);

		// briskDetector->create("Feature2D.BRISK");

		// Step -1, Detect keypoints using SURF detector
		briskDetector->detect(img_reference, keypoints_reference);
		briskDetector->detect(img_target, keypoints_target);
		// Step -2, Calculate descriptors (feature vector)
		briskDetector->compute(img_reference, keypoints_reference, descriptors_reference);
		briskDetector->compute(img_target, keypoints_target, descriptors_target);
	}

	else if (siftOrSurf == 3)
	{
		Ptr<SURF> detector = SURF::create(minHessian, 2, 1, true, false);
		// SurfFeatureDetector detector = SurfFeatureDetector(minHessian, 2, 1, true, false);
		detector->detect(img_reference, keypoints_reference);
		detector->detect(img_target, keypoints_target);

		Ptr<FREAK> extractor = FREAK::create(false, false, 22.0f, 4);
		extractor->compute(img_reference, keypoints_reference, descriptors_reference);
		extractor->compute(img_target, keypoints_target, descriptors_target);
	}
}
...
void desenhando(Mat &imgDupla, std::vector<Point2f> &bestInX1Vec, std::vector<Point2f> &bestInX2Vec)
{
	for (unsigned int k = 0; k < bestInX1Vec.size(); k++)
	{
		...
		line(imgDupla, ponto1, ponto2, corLinha, 1, LINE_AA);
		circle(imgDupla, ponto1, 3, corLinha, 1, LINE_AA);
		circle(imgDupla, ponto2, 3, corLinha, 1, LINE_AA);
	}
}
...
void geraVetoresEnviarCalcularHomografia(int scaleDependent, int usaRestricao1grau, int usaMediaDistancia, int contadorMax, double distMult, std::vector<DMatch> &good_matches, double intervaloTamanho, int soEixoX, double controlaIntervaloAngulo, unsigned int numMinimoPares, int usaTotal, int siftOrSurf, int minHessian, Mat &img_reference, std::vector<KeyPoint> &keypoints_reference, Mat &descriptors_reference, Mat &img_target, std::vector<KeyPoint> &keypoints_target, Mat &descriptors_target, std::vector<Point2f> &reference_enviar, std::vector<Point2f> &target_enviar)
{
	...
	while (contadorLoop < contadorMax)
	{
		...
		detect_feature_extract_descriptor(siftOrSurf, minHessian, img_reference, keypoints_reference, descriptors_reference, img_target, keypoints_target, descriptors_target);
		...
		if ((siftOrSurf == 0) || (siftOrSurf == 1))
		{
			matcher1.match(descriptors_reference, descriptors_target, matches);
		}
		else if ((siftOrSurf == 2) || (siftOrSurf == 3))
		{
			matcher2.match(descriptors_reference, descriptors_target, matches);
		}
		...
		// Mat imgDupla_antes;
		// geraImagemDupla(img_reference, img_target, imgDupla_antes);
		// desenhando(imgDupla_antes, keypoints_reference, keypoints_target);
		...
		for (unsigned int i = 0; i < good_matches.size(); i++)
		{
			...
			if (scaleDependent == 1)
			{
				...
				if (keypoints_reference[good_matches[i].queryIdx].octave == keypoints_target[good_matches[i].trainIdx].octave)
				{
					reference.push_back(keypoints_reference[good_matches[i].queryIdx].pt);
					target.push_back(keypoints_target[good_matches[i].trainIdx].pt);
				}
		...
		if (usaMediaDistancia == 1)
		{
			std::vector<Point2f> reference_distancia;
			std::vector<Point2f> target_distancia;

			if (reference_enviar.size() > 1)
			{
				double distAux2 = (double)img_reference.cols;
				removeDistanciaDispar(reference_enviar, target_enviar, reference_distancia, target_distancia, distAux2, intervaloTamanho, soEixoX);
...
void desenhaHomografiaImgMatches(Mat &img_matches, int numColunas, int numLinhas, Mat &H)
{
	...
	reference_corners[0] = Point(0, 0);
	reference_corners[1] = Point(numColunas, 0);
	reference_corners[2] = Point(numColunas, numLinhas);
	reference_corners[3] = Point(0, numLinhas);
...
void gera_mat_detect_final_multirresolucao(Mat &fusao_mascaras_voting, std::vector<Mat> &count_mask_vec, double delta_janela, std::vector<double> multiplica_area_janela_vec, std::vector<int> dimensaoJanelaNCC_vec, std::vector<unsigned int> subamostraPorNCC_vec, unsigned int num_multres, int threshold_multirresolucao, Mat img_target_big, unsigned int usaRectouAreaDaMancha, unsigned int divisorShow)
{
	...
	for (unsigned int auxLoop; auxLoop < num_multres; auxLoop++)
	{
		...
		sprintf(nomeArquivoRes, "/home/luiz.tavares/Workspace/VDAO_Pixel/scripts/extra/ADMULT/results/manchasNoSelect_res%u.yuv", auxLoop);
	...
	for (unsigned int contAuxImprime = 0; contAuxImprime < num_multres; contAuxImprime++)
	{
		...
		sprintf(nomeArquivo, "/home/luiz.tavares/Workspace/VDAO_Pixel/scripts/extra/ADMULT/results/quantidade_objs_res_%u.txt", contAuxImprime);
		...
		sprintf(nomeArquivoMorphRes, "/home/luiz.tavares/Workspace/VDAO_Pixel/scripts/extra/ADMULT/results/manchas_res%u.yuv", contAuxImprime);
...
void desenhaHomografiaImgMatches_bordaPreta(Mat &img_matches_old, int numColunas, int numLinhas, Mat &H, Mat &img_matches_retornar)
{
	std::vector<Point2f> reference_corners(4);
	reference_corners[0] = Point(0, 0);
	reference_corners[1] = Point(numColunas, 0);
	...
	cvtColor(img_matches_old, desenhaTemp, COLOR_BGR2GRAY);

	Mat desenhaTemp2;

	int borda = 60; // deve ser par

	desenhaTemp2.create(desenhaTemp.rows + borda, desenhaTemp.cols + borda, desenhaTemp.type());

	for (int k = 0; k < desenhaTemp2.rows; k++)
	{
		for (int l = 0; l < desenhaTemp2.cols; l++)
		{
			desenhaTemp2.at<unsigned char>(k, l) = (unsigned char)0;
		}
	}

	for (int k = borda / 2; k < (desenhaTemp2.rows - borda / 2); k++)
	{
		for (int l = borda / 2; l < (desenhaTemp2.cols - borda / 2); l++)
		{
			desenhaTemp2.at<unsigned char>(k, l) = desenhaTemp.at<unsigned char>(k - borda / 2, l - borda / 2);
		}
	}

	// imshow("Vai Gravar Temp", desenhaTemp2);
	// waitKey(0);

	Mat img_matches;

	cvtColor(desenhaTemp2, img_matches, COLOR_GRAY2BGR);

	line(img_matches, target_corners[0] + Point2f(numColunas, 0) + Point2f(borda / 2, borda / 2), target_corners[1] + Point2f(numColunas, 0) + Point2f(borda / 2, borda / 2), Scalar(0, 255, 0), 4);
	line(img_matches, target_corners[1] + Point2f(numColunas, 0) + Point2f(borda / 2, borda / 2), target_corners[2] + Point2f(numColunas, 0) + Point2f(borda / 2, borda / 2), Scalar(0, 255, 0), 4);
	line(img_matches, target_corners[2] + Point2f(numColunas, 0) + Point2f(borda / 2, borda / 2), target_corners[3] + Point2f(numColunas, 0) + Point2f(borda / 2, borda / 2), Scalar(0, 255, 0), 4);
	line(img_matches, target_corners[3] + Point2f(numColunas, 0) + Point2f(borda / 2, borda / 2), target_corners[0] + Point2f(numColunas, 0) + Point2f(borda / 2, borda / 2), Scalar(0, 255, 0), 4);

	img_matches_retornar = img_matches.clone();

	// imshow("Vai Gravar", img_matches);
	// waitKey(0);
}

void temporalFilter(std::vector<Mat> &vecNCC, unsigned int tamVecNCC, std::vector<Mat> &vecH, Mat &escalaHomografia1, Mat &escalaHomografia2, std::vector<Mat> &vecTF, unsigned int tamVecTF)
{

	Mat matTF = vecNCC[tamVecNCC - 1].clone();
	int contadorHintra = vecH.size() - 1;
	Mat Haux = Mat::eye(vecH[0].size(), vecH[0].type());

	Mat verificaTFOriginal;
	resize(matTF, verificaTFOriginal, Size(matTF.cols * 2, matTF.rows * 2));
	// imshow("TestaTFor", verificaTFOriginal);
	// waitKey(0);

	for (int count = tamVecNCC - 2; count >= 0; count--)
	{
		Mat aux1 = vecNCC[count].clone();
		// Haux = vecH[contadorHintra]*Haux;
		Haux = Haux * vecH[contadorHintra];
		// Mat HauxEsc = escalaHomografia1*Haux*escalaHomografia2;
		Mat HauxEsc = Haux.clone();
		Mat aux1Warped;
		warpPerspective(aux1, aux1Warped, HauxEsc, aux1.size());
		contadorHintra--;

		Mat MaskAuxHintraTemp;
		MaskAuxHintraTemp.create(vecNCC[0].size(), vecNCC[0].type());
		criaWarpMask(MaskAuxHintraTemp, HauxEsc);

		Mat M2warpMask;
		corrigeVotingMask(aux1Warped, M2warpMask, MaskAuxHintraTemp);

		Mat intersecIdp;
		intersecIdp.create(vecNCC[0].size(), vecNCC[0].type());

		// timeval tv9, tv10;
		// gettimeofday(&tv9, NULL);
		calculaMatIntersecao(intersecIdp, matTF, M2warpMask);
		// gettimeofday(&tv10, NULL);

		// printf("Tempo da intersecao filtragem temporal: %lf \n", tv10.tv_sec - tv9.tv_sec + (tv10.tv_usec - tv9.tv_usec)/1e6);
		// fflush(stdout);

		printf("Indice vetor tf: %d \n", count);
		printf("Tamanho real vetor tf: %lu \n", vecNCC.size());
		printf("Tamanho definido vetor tf: %u \n", tamVecNCC);

		printf("Indice vetor H: %d \n", contadorHintra + 1);
		printf("Tamanho real vetor H: %lu \n \n", vecH.size());
		// printf("Tamanho definido vetor H: \n", tamVecH);
		fflush(stdout);

		Mat verificaTFfim;
		resize(intersecIdp, verificaTFfim, Size(matTF.cols * 2, matTF.rows * 2));
		// imshow("TestaTFfim", verificaTFfim);
		// waitKey(0);

		intersecIdp.copyTo(matTF);
	}

	// imshow("matTF", matTF);
	waitKey(1);

	Mat matTFaux = matTF.clone();
	vecTF.push_back(matTFaux);
	if (vecTF.size() > tamVecTF)
	{
		vecTF.erase(vecTF.begin());
	}
}

void votingStep(std::vector<Mat> &vecTF, unsigned int tamVecTF, std::vector<Mat> &vecH, Mat &escalaHomografia1, Mat &escalaHomografia2, unsigned int votThreshold, Mat &votResult)
{

	Mat matVot = vecTF[tamVecTF - 1].clone();
	int contadorHintra = vecH.size() - 1;
	Mat Haux = Mat::eye(vecH[0].size(), vecH[0].type());

	Mat count_matrix;
	count_matrix.create(Size((vecTF[0].cols), (vecTF[0].rows)), CV_8U);
	Mat count_mask;
	count_mask.create(Size((vecTF[0].cols), (vecTF[0].rows)), DataType<unsigned char>::type);
	for (int k = 0; k < count_matrix.rows; k++)
	{
		for (int l = 0; l < count_matrix.cols; l++)
		{
			...
			double comparaVotingAux = (double)matVot.at<unsigned char>(k, l);

			if ((comparaVotingAux != 255.0) && (comparaVotingAux != 0.0))
			{
				count_matrix.at<unsigned int>(k, l) = 1;
			}
			else
			{
				count_matrix.at<unsigned int>(k, l) = 0;
	...
	for (int count = tamVecTF - 2; count >= 0; count--)
	{
		Mat aux1 = vecTF[count].clone();
		// Haux = vecH[contadorHintra]*Haux;
		Haux = Haux * vecH[contadorHintra];
		// Mat HauxEsc = escalaHomografia1*Haux*escalaHomografia2;
		Mat HauxEsc = Haux.clone();
		...
		Mat MaskAuxHintraTemp;
		MaskAuxHintraTemp.create(vecTF[0].size(), vecTF[0].type());
		criaWarpMask(MaskAuxHintraTemp, HauxEsc);

		Mat M2warpMask;
		corrigeVotingMask(aux1Warped, M2warpMask, MaskAuxHintraTemp);
		// cout << M2warpMask.size() << ", " << count_matrix.rows << ", " << count_matrix.cols << endl;
		//  cv::transpose(M2warpMask, M2warpMask);

		for (int k = 0; k < count_matrix.rows; k++)
		{
			for (int l = 0; l < count_matrix.cols; l++)
			{
				double comparaVotingAux = (double)M2warpMask.at<unsigned char>(k, l);
	...
	for (int k = 0; k < count_matrix.rows; k++)
	{
		for (int l = 0; l < count_matrix.cols; l++)
		{
			if (count_matrix.at<unsigned int>(k, l) >= votThreshold)
			{
	...
	// imshow("count_mask", count_mask);
	// waitKey(1);
	// cout << "Here 1.5 " <<  endl;

	count_mask.copyTo(votResult);
}

void Erosion(int erosion_elem, int erosion_size, Mat &src, Mat &erosion_dst)
{
	int erosion_type;
	if (erosion_elem == 0)
	{
...
void Dilation(int dilation_elem, int dilation_size, Mat &src, Mat &dilation_dst)
{
	int dilation_type;
	if (dilation_elem == 0)
	{
...
void Fechamento(int fechamento_elem, int fechamento_size, Mat &src, Mat &fechamentoOutput)
{

	int fechamento_type;
	if (fechamento_elem == 0)
	{
		fechamento_type = MORPH_RECT;
	}
	else if (fechamento_elem == 1)
	{
		fechamento_type = MORPH_CROSS;
	}
	else if (fechamento_elem == 2)
	{
		fechamento_type = MORPH_ELLIPSE;
	}

	Mat element_aux01 = getStructuringElement(fechamento_type,
											  Size(2 * fechamento_size + 1, 2 * fechamento_size + 1),
											  Point(fechamento_size, fechamento_size));

	Mat erosion_dst;

	Erosion(fechamento_elem, fechamento_size, src, erosion_dst);
	Dilation(fechamento_elem, fechamento_size, erosion_dst, fechamentoOutput);
}

void Abertura(int abertura_elem, int abertura_size, Mat &src, Mat &aberturaOutput)
{

	int abertura_type;
	if (abertura_elem == 0)
	{
		abertura_type = MORPH_RECT;
	}
	else if (abertura_elem == 1)
	{
		abertura_type = MORPH_CROSS;
	}
	else if (abertura_elem == 2)
	{
		abertura_type = MORPH_ELLIPSE;
	}

	Mat element_aux01 = getStructuringElement(abertura_type,
											  Size(2 * abertura_size + 1, 2 * abertura_size + 1),
											  Point(abertura_size, abertura_size));

	Mat dilation_dst;

	Dilation(abertura_elem, abertura_size, src, dilation_dst);
	Erosion(abertura_elem, abertura_size, dilation_dst, aberturaOutput);
}

void AbreFecha(int abertura_elem, int abertura_size, int fechamento_elem, int fechamento_size, Mat &src, Mat &AbreFechaOutput)
{
	Mat aberturaOutput;

	Abertura(abertura_elem, abertura_size, src, aberturaOutput);
	Fechamento(fechamento_elem, fechamento_size, aberturaOutput, AbreFechaOutput);
}

void FechaAbre(int abertura_elem, int abertura_size, int fechamento_elem, int fechamento_size, Mat &src, Mat &FechaAbreOutput)
{
	Mat fechamentoOutput;

	Fechamento(fechamento_elem, fechamento_size, src, fechamentoOutput);
	Abertura(abertura_elem, abertura_size, fechamentoOutput, FechaAbreOutput);
}

void morphOps(int salvaQual, int abertura_elem, int abertura_size, int fechamento_elem, int fechamento_size, Mat &src, Mat &morphOpOutput)
{

	if (salvaQual == 1)
	{
		FechaAbre(abertura_elem, abertura_size, fechamento_elem, fechamento_size, src, morphOpOutput);
	}
	else
	{
		AbreFecha(abertura_elem, abertura_size, fechamento_elem, fechamento_size, src, morphOpOutput);
	}
}

//******************************************************************************
// PROCEDURE normalizeIllum
//******************************************************************************
//
...
void normalizeIllum(const cv::Mat &matSrc, cv::Mat &matDst,
					const cv::Mat &matMsk)
{
	double dGamma = 0.2;
	...
	matA.create(matSrc.size(), CV_32FC1);
	matB.create(matSrc.size(), CV_32FC1);

	// Gamma correction.

	matSrc.convertTo(matA, matA.type());
	cv::pow(matA, dGamma, matA);

	matB = matA.clone();

	// Difference of Gaussians (DoG).

	cv::GaussianBlur(matA, matA, cv::Size(0, 0), dSigmaA, 0.0);
	cv::GaussianBlur(matB, matB, cv::Size(0, 0), dSigmaB, 0.0);

	// cv::subtract(matA, matB, matA, matMsk);
	cv::subtract(matA, matB, matA);
	// Contrast equalization.

	// Step 1.
	cv::pow(matA, dAlpha, matB);
	cout << matA << endl;

	// dMean = cv::mean(matB, matMsk)[0];
	dMean = cv::mean(matB)[0];

	matB = cv::abs(matA);
	matB = cv::min(matB, dTau);
	cv::pow(matB, dAlpha, matB);

	dMean = cv::mean(matB, matMsk)[0];
	matA.convertTo(matA, matA.type(), 2.0 / (dTau * pow(dMean, 1.0 / dAlpha)));

	// Step 3 (tau*tanh( A/tau )).
	cv::exp(matA, matA);

	matB = cv::Mat::zeros(matB.size(), matB.type());
	matA.copyTo(matB, matMsk);

	cv::subtract(matB, cv::Scalar(1.0), matA, matMsk);
	cv::add(matB, cv::Scalar(1.0), matB, matMsk);

	cv::divide(matA, matB, matA, dTau);

	// End.
	matA.convertTo(matDst, matDst.type());
} // End of normalizeIllum.

//******************************************************************************
// PROCEDURE normalizeMeanVar
//******************************************************************************
//
// Scales the pixels so that they have zero mean and unitary variance.
//
//******************************************************************************

void normalizeMeanVar(const cv::Mat &matSrc, cv::Mat &matDst,
					  const cv::Mat &matMsk)
{
	double dVal;
	...
	cv::meanStdDev(matSrc, sclMean, sclSDev, matMsk);
	...
	matAux.create(matSrc.size(), CV_64FC1);

	if (true == matMsk.empty())
	{
		for (int y = 0; y < matSrc.rows; ++y)
			for (int x = 0; x < matSrc.cols; ++x)
			{
				dVal = (matSrc.at<double>(y, x) - dMean) * dInvSDev;

				matAux.at<double>(y, x) = dVal;
			}
	}
	else
	{
		...
		for (int y = 0; y < matSrc.rows; ++y)
			for (int x = 0; x < matSrc.cols; ++x)
			{
				iVal = int(matMsk.at<uchar>(y, x) + 0.5);

				dVal = (1 == iVal)
						   ? (matSrc.at<double>(y, x) - dMean) * dInvSDev
						   : 0.0;
	...
	matAux.convertTo(matDst, matDst.type());
} // End of normalizeMeanVar.

//******************************************************************************
// PROCEDURE normalizeImage
//******************************************************************************

void normalizeImage(const cv::Mat &matSrc, cv::Mat &matDst,
					const cv::Mat &matMsk)
{
	cv::Mat matAux;
	...
	normalizeIllum(matSrc, matAux, matMsk);

	normalizeMeanVar(matAux, matAux, matMsk);

	matAux.convertTo(matDst, matDst.type(), 42.5, 127.5);

	return;

	double dSigma = 3.0;

	cv::Laplacian(matSrc, matAux, matAux.type(), 3);
	cv::Laplacian(matSrc, matAux, matAux.type(), 3);
	cv::convertScaleAbs(matAux, matDst, (dSigma + 1.0) * 0.25);
} // End of normalizeImage.
