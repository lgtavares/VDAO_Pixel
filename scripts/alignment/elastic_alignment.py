...
# Loading dataset
vdao_geometric = VideoVDAODataset(
    fold_number=fold_num,
    split_number=0,
    align_file=os.path.join(
        PROJECT_DIR,
        "data/alignment/geometric_{0}_fold{1:02d}.csv".format(fold_type,
                                                              fold_num),
    ),
...
for vid in videos:
    ...
    for frame in vid_index:
        ...
        v, u = optical_flow_tvl1(
            tar_frame, ref_frame, tightness=0.1, num_warp=20, tol=1e-7
        )
        ...
        cv2.imwrite(
            "/nfs/proc/luiz.tavares/VDAO_Database/data/{2}/" +
            "ref_warp/fold{0:02d}/{1:04d}.png".format(
                fold_num, frame, fold_type
            ),
            cv2.cvtColor(wref, cv2.COLOR_RGB2BGR),
        )

        with open(
            "/nfs/proc/luiz.tavares/VDAO_Database/data/{0}_debug.txt".format(fold_type),
            "a",
        ) as debug_file:
