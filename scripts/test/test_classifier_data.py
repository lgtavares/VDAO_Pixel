...
for fold in range(1, 10):
    ...
    lgbm_trn = os.path.join(
        out_dir, '{0}_{1}_fold{2:02d}_trn.pkl'.format(classifier, alignment,
                                                      fold))
    ...
    full = lgb.Dataset(data=merge_df.iloc[:, 0:512].values,
    ...
    if not os.path.exists(lgbm_trn):
        ...
        train = lgb.Dataset(data=train_df.iloc[:, 0:512].values,
        ...
        val = lgb.Dataset(data=val_df.iloc[:, 0:512].values,
...
conda activate pixel_env; nohup nice -n 19 python3 \
    ~/Workspace/VDAO_Pixel/scripts/test/test_classifier_data.py > rf_err.out &
