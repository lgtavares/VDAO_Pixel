...
if save_results:
    save_dict_file = os.path.join(
        RESULT_DIR, "test_results", "{0}_{1}.pkl".format(classifier, alignment)
    )
    temp_dict = {}
...
# Training
dataset = VDAODataset(
    fold=fold, split=-1, type="test", alignment=alignment, transform=True
)
...
for i, (ref_frame, tar_frame,
        sil_frame, info) in enumerate(tqdm(loader, desc="batch")):
    # Info
...
with open(os.path.join(class_dir,
                       'time_fold{0}.txt'.format(fold)), 'w+') as temp_file:
    temp_file.write('Elapsed time: {0}.\n'.format(delta_time))
# pickle.dump(out_dict, open(out_file, "wb"))

if save_results:
    res_params = pickle.load(
        open(os.path.join(RESULT_DIR, "hyperparameter_search.pkl"), "rb")
    )
    ...
    for vv in videos:
        ...
        vid = voting_window(
            vid, (voting_depth, voting_width, voting_width), count / 100
        )
...
conda activate pixel_env; nohup nice -n 19 python3\
    ...
    --class LightGBM --align geometric --fold 1 > lgbm_data.out &

conda activate pixel_env; nohup nice -n 19 python3\
    ~/Workspace/VDAO_Pixel/scripts/test/classifier_outputs.py \
    --class LightGBM --align geometric --fold 2 > lgbm_data.out &

conda activate pixel_env; nohup nice -n 19 python3\
    ~/Workspace/VDAO_Pixel/scripts/test/classifier_outputs.py \
    --class LightGBM --align geometric --fold 3 > lgbm_data.out &

conda activate pixel_env; nohup nice -n 19 python3\
    ~/Workspace/VDAO_Pixel/scripts/test/classifier_outputs.py \
    --class LightGBM --align geometric --fold 4 > lgbm_data.out &

conda activate pixel_env; nohup nice -n 19 python3\
    ~/Workspace/VDAO_Pixel/scripts/test/classifier_outputs.py \
    --class LightGBM --align geometric --fold 5 > lgbm_data.out &

conda activate pixel_env; nohup nice -n 19 python3\
    ~/Workspace/VDAO_Pixel/scripts/test/classifier_outputs.py \
    --class LightGBM --align geometric --fold 6 > lgbm_data.out &

conda activate pixel_env; nohup nice -n 19 python3\
    ~/Workspace/VDAO_Pixel/scripts/test/classifier_outputs.py \
    --class LightGBM --align geometric --fold 7 > lgbm_data.out &

conda activate pixel_env; nohup nice -n 19 python3\
    ~/Workspace/VDAO_Pixel/scripts/test/classifier_outputs.py \
    --class LightGBM --align geometric --fold 8 > lgbm_data.out &

conda activate pixel_env; nohup nice -n 19 python3\
    ~/Workspace/VDAO_Pixel/scripts/test/classifier_outputs.py \
    --class LightGBM --align geometric --fold 9 > lgbm_data.out &
"""
# Fold 1: node-02-01
# Fold 2: node-02-02
# Fold 3: node-02-03
